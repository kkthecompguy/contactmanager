...
class Contact extends Component {
  ...
  onDeleteClick = async (id, dispatch) => {
    try {
      await axios.delete(`https://jsonplaceholder.typicode.com/users/${this.id}`);
        ...
        {value => {
          ...
          return (
            <div className="card card-body mb-3">
              <h4>{name}
                <i onClick={this.onShowClick = () => this.setState({ showContactInfo: !this.state.showContactInfo })} className="fas fa-sort-down" style={{ cursor: 'pointer' }}></i>
